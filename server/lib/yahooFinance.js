...
async function getClosestPrice(ticker, targetDate) {
  ...
  // targetDate에 가장 가까운 날짜의 종가 반환
  const target = new Date(targetDate).getTime();
  const closest = history.reduce((prev, cur) => {
    return Math.abs(new Date(cur.date).getTime() - target) <
...
function buildQuizOptions(correctRate) {
  ...
  wrongs.push(Math.round((Math.abs(correct) * 0.6 + 2) * opposite * 100) / 100);
  wrongs.push(Math.round((Math.abs(correct) * 1.4 + 5) * opposite * 100) / 100);

  // 같은 방향이지만 수치가 크게 다른 오답 1개
  const sameDir = correct >= 0 ? 1 : -1;
  wrongs.push(Math.round((Math.abs(correct) * 2.2 + 8) * sameDir * 100) / 100);
