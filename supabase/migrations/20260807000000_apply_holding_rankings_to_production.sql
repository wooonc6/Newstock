-- 2026-08-07: make production ranking and public portfolio snapshots expose active holdings.
-- GitHub migrations are not applied by Vercel deploys, so keep this migration explicit.

drop function if exists public.get_learning_rankings(integer);

create function public.get_learning_rankings(p_limit integer default 500)
returns table (
  id uuid,
  nickname text,
...
begin
  ...
  select
    ...
    p.updated_at
  from public.users as u
  left join public.portfolio as p on p.user_id = u.id and p.quantity > 0
