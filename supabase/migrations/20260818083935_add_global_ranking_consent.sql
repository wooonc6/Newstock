-- Each class join records whether the learner agrees to appear globally.
-- Existing memberships retain the previous behavior (visible globally).

alter table public.class_members
  add column show_in_global_ranking boolean not null default true;

drop function if exists public.join_class_by_code(text);
create function public.join_class_by_code(p_class_code text, p_global_ranking_consent boolean)
...
begin
  ...
  if p_global_ranking_consent is null then raise exception 'Ranking consent required' using errcode = '22023'; end if;
  select c.* into v_class from public.classes c where upper(c.class_code) = upper(trim(p_class_code));
  if v_class.id is null then raise exception 'Invalid class code' using errcode = 'P0002'; end if;
  insert into public.class_members (class_id, user_id, show_in_global_ranking)
  values (v_class.id, v_user_id, p_global_ranking_consent)
  on conflict (class_id, user_id) do update
    set show_in_global_ranking = excluded.show_in_global_ranking
  returning class_members.joined_at into v_joined_at;
  return query select v_class.id, v_class.name, v_joined_at;
end;
$$;
revoke all on function public.join_class_by_code(text, boolean) from public, anon;
grant execute on function public.join_class_by_code(text, boolean) to authenticated;
...
begin
  ...
  if p_class_id is not null and not exists (
    select 1 from public.class_members cm where cm.class_id = p_class_id and cm.user_id = (select auth.uid())
  ) then raise exception 'Class membership required' using errcode = '42501'; end if;
  ...
  with quiz_totals as (
    ...
    select t.user_id, coalesce(sum(t.realized_profit), 0)::numeric realized_profit,
    ...
    select p.user_id,
      ...
      coalesce(jsonb_agg(jsonb_build_object('ticker', p.ticker, 'quantity', p.quantity, 'avg_cost', p.avg_cost, 'updated_at', p.updated_at) order by p.ticker) filter (where p.quantity > 0), '[]'::jsonb) holdings
    from public.portfolio p group by p.user_id
  )
  select u.id, coalesce(nullif(u.nickname, ''), nullif(split_part(u.email, '@', 1), '')),
    coalesce(u.coins, 0)::integer, coalesce(q.total_earned_coins, 0)::bigint,
    coalesce(r.realized_profit, 0)::numeric, coalesce(r.realized_cost_basis, 0)::numeric,
    case when coalesce(r.realized_cost_basis, 0) > 0 then round((r.realized_profit / r.realized_cost_basis) * 100, 2) else 0::numeric end,
    coalesce(p.portfolio_cost_basis, 0)::numeric, coalesce(p.holding_count, 0)::bigint,
