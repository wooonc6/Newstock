...
export default function GrowthTimeline({ points }: { points: TimelinePoint[] }) {
  ...
  const linePath = filtered
    .map((p, i) => `${i === 0 ? "M" : "L"} ${xFor(new Date(p.date).getTime()).toFixed(1)} ${yFor(p.cumulativeProfit).toFixed(1)}`)
    .join(" ");
  const zeroY = yFor(0);
  ...
  return (
    <div>
        ...
        <svg viewBox={`0 0 ${width} ${height}`} style={{ width: "100%", minWidth: 560, height: "auto", display: "block" }}>
          ...
          {filtered.map((p, i) => {
            ...
            return (
              ...
              >
                <circle cx={x} cy={y} r={selected ? 8 : isTrade ? 5 : 6} fill={markerColor} stroke="var(--surface)" strokeWidth={selected ? 3 : 2} />
                {isTrade ? (
      ...
      <div style={{ marginTop: 10, fontSize: 12, color: "var(--text-dim)", minHeight: 18, lineHeight: 1.55 }}>
        {active ? (
          <span>
            <strong style={{ color: "var(--text)" }}>{active.name}</strong> · {active.detail} · 누적 실현손익 {formatSignedWon(active.cumulativeProfit)}
          </span>
        ) : (
          <span>그래프의 점을 터치하거나 마우스를 올리면 거래 상세를 볼 수 있어요. B=매수, S=매도, 마지막 점=현재 평가</span>
        )}
      </div>
    </div>
  );
