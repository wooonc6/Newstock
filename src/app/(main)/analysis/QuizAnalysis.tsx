...
import CompetencyReportSection from "./CompetencyReport";
import {
...
export default async function QuizAnalysis() {
  ...
  if (!user) return <EmptyPanel message="로그인 후 퀴즈 분석을 확인할 수 있습니다." />;
  ...
  return (
    <div style={{ display: "grid", gap: 20 }}>
      <section className="mobile-section" style={{ border: "1px solid var(--border)", borderRadius: 18, background: "var(--surface)", padding: 24 }}>
      ...
      <CompetencyReportSection report={competency} />
            ...
            {sectors.map((sector) => (
              <div key={sector.sector}>
                <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 5, fontSize: 12 }}>
                  <span style={{ fontWeight: 700 }}>{sector.sector}</span>
      ...
      <section className="mobile-section" style={{ background: "var(--surface)", border: "1px solid var(--border)", borderRadius: 14, padding: 18 }}>
        ...
        {weakConcepts.length === 0 ? (
          <div style={{ fontSize: 13, color: "var(--text-muted)" }}>아직 틀린 문제가 없습니다. 정답률이 아주 좋아요!</div>
        ) : (
          <div style={{ display: "grid", gap: 8 }}>
            {weakConcepts.map((concept, index) => (
              <div key={concept.category} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: 12, padding: "10px 12px", background: "var(--surface2)", borderRadius: 8 }}>
                <span style={{ fontSize: 13, fontWeight: 700 }}>{index + 1}. {concept.category}</span>
                <span style={{ fontSize: 12, color: "var(--danger)", fontWeight: 800, whiteSpace: "nowrap" }}>{concept.wrongCount}회 오답</span>
              </div>
            ))}
          </div>
        )}
    ...
    <div style={{ overflowX: "auto" }}>
      <svg viewBox={`0 0 ${width} ${height}`} style={{ width: "100%", minWidth: 520, height: "auto", display: "block" }}>
        ...
        {points.map((point, index) => {
          const barHeight = (Math.max(0, Math.min(100, point.accuracyPct)) / 100) * (height - padY * 2);
          const x = padX + index * (barWidth + barGap);
          const y = height - padY - barHeight;
          const color = point.total === 0 ? "var(--border)" : point.accuracyPct >= 60 ? "var(--accent)" : "var(--danger)";
          return (
            <g key={point.weekLabel}>
              <rect x={x} y={y} width={barWidth} height={Math.max(2, barHeight)} rx={3} fill={color} />
              {point.total > 0 ? <text x={x + barWidth / 2} y={Math.max(11, y - 5)} textAnchor="middle" fontSize={9} fontWeight={800} fill="var(--text-dim)">{Math.round(point.accuracyPct)}%</text> : null}
              <text x={x + barWidth / 2} y={height - 4} textAnchor="middle" fontSize={8} fill="var(--text-muted)">{point.weekLabel}</text>
            </g>
          );
        })}
      </svg>
...
function Kpi({ label, value, tone }: { label: string; value: string; tone?: "up" | "down" }) {
  ...
  return <div style={{ background: "var(--surface)", border: "1px solid var(--border)", borderRadius: 10, padding: 14 }}><div style={{ fontSize: 10, color: "var(--text-muted)", marginBottom: 6 }}>{label}</div><div style={{ fontSize: 17, fontWeight: 800, color }}>{value}</div></div>;
}
