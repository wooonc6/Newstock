    ...
    async function fetchNews() {
      ...
      try {
        ...
        if (!response.ok) {
          ...
          throw new Error(
            errorData?.error ?? `뉴스 요청에 실패했습니다. (${response.status})`
          );
        }
      ...
      <section style={{ display: "grid", gap: "9px" }}>
        {sectorGroups.map(([sector, stocks], groupIndex) => {
          const totalNews = stocks.reduce(
            (sum, stock) => sum + (newsCounts[stock.ticker] ?? 0),
            0
          );
              ...
              >
                {stocks.map((stock) => {
                  ...
                  const remaining = Math.max(
                    status.quizzes_required - status.quizzes_completed,
                    0
                  );
                      ...
                      >
                        <div style={{ display: "flex", justifyContent: "space-between", gap: "10px" }}>
                          <div style={{ minWidth: 0 }}>
                            <strong style={{ fontSize: "14px", lineHeight: 1.35 }}>
                              {stock.name}
                            </strong>
            ...
            {recentNews.length === 0 ? (
              ...
              recentNews.map((news) => (
                ...
                >
                  ...
                  <div style={{ fontSize: "13px", fontWeight: 700, lineHeight: 1.45 }}>
                    {news.title}
                  </div>
                </div>
              ))
