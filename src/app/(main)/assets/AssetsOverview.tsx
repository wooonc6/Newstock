    ...
    >
      <div style={{ display: "flex", justifyContent: "space-between", gap: "12px", alignItems: "start" }}>
        ...
        <div style={{ textAlign: "right" }}>
          <div style={{ fontSize: "14px", fontWeight: 800 }}>{loading ? "시세 조회 중..." : price > 0 ? `₩${value.toLocaleString()}` : "현재가 확인 불가"}</div>
          {!loading && price > 0 ? (
            <div style={{ marginTop: "4px", fontSize: "11px", fontWeight: 800, color: positive ? "#ef4444" : "#2563eb" }}>
              {positive ? "+" : ""}{profit.toLocaleString()}원 ({positive ? "+" : ""}{profitRate.toFixed(1)}%)
            </div>
          ) : null}
          {!loading && price <= 0 ? (
            <div style={{ marginTop: "4px", fontSize: "10px", color: "var(--text-muted)" }}>평가손익은 시세 복구 후 표시됩니다.</div>
          ) : null}
        </div>
      </div>
    </Link>
  ...
  return (
    <div style={{ display: "grid", gap: "16px" }}>
      <section className="mobile-section" style={{ background: "var(--surface)", border: "1px solid var(--border)", borderRadius: "12px", padding: "18px" }}>
        ...
        <div className="assets-summary-grid" style={{ marginTop: "16px", display: "grid", gridTemplateColumns: "repeat(3, minmax(0, 1fr))", gap: "8px" }}>
      ...
      <style jsx>{`
        @media (max-width: 640px) {
          ...
          .assets-summary-grid > :first-child {
            grid-column: 1 / -1;
          }
        }
      `}</style>
...
function Metric({ label, value }: { label: string; value: string }) {
  return <div style={{ padding: "12px", background: "var(--surface2)", borderRadius: "8px", minWidth: 0 }}><div style={{ fontSize: "10px", color: "var(--text-muted)", marginBottom: "5px" }}>{label}</div><div style={{ fontSize: "14px", fontWeight: 800, overflowWrap: "anywhere" }}>{value}</div></div>;
}
