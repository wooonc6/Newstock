...
- 초기 시드 머니: 모든 사용자 동일 (예: 1,000,000 코인)
...
CREATE TABLE quiz_questions (
  ...
  stock_ticker TEXT,          -- 예: '005930.KS'
  news_title TEXT,
  news_url TEXT,
  news_date DATE,             -- 뉴스 발생일
  price_before NUMERIC,       -- 발생일 종가
  price_after NUMERIC,        -- 다음날 종가
...
- 각 테이블 RLS(Row Level Security) 설정 — 본인 데이터만 읽기/쓰기
...
목표: 네이버 뉴스 API로 1/3/6개월 전 종목별 뉴스를 수집하고, 야후 주가로 정답(상승/하락)을 자동 판정해서 `quiz_questions` 테이블에 저장

```
GET /api/news/collect?ticker=005930.KS&period=1m
  → 네이버 뉴스 API로 1개월 전 삼성전자 뉴스 수집
  → yahoo-finance2로 해당 날짜 주가 조회
  → price_before vs price_after 비교 → answer = 'up' or 'down'
  → quiz_questions 테이블에 저장
```

**`feat/unlock-backend` — 퀴즈 채점 + 언락 처리**

```
POST /api/quiz/submit
  body: { question_id, user_answer }
  → 정답 확인
  → quiz_sessions에 결과 저장
  → 해당 종목 3문제 모두 맞히면 포트폴리오 접근 허용
```

**`feat/trade-logic` — 매수/매도 로직**
...
`next.config.mjs`에서 yahoo-finance2 webpack 에러 해결
- 현재 `@gadicc/fetch-mock-cache/runtimes/deno.ts` 에러 발생 중

**`feat/news-feed-ui` — 퀴즈 페이지 UI**

`src/app/(main)/quiz/[stock]/page.tsx`에 구현:
- 뉴스 기사 제목 / 날짜 표시
- "주가 상승" / "주가 하락" 선택 버튼
- 정답 후 실제 주가 변동 애니메이션 표시

**`feat/ranking-ui` — 랭킹 페이지**

`src/app/(main)/ranking/page.tsx`에 구현:
- 전체 사용자 포트폴리오 수익률 순위
- 주간 / 전체 탭 전환
- 상위 3위 강조 표시
...
**`feat/design-system` — 공통 컴포넌트 정리**

`src/components/ui/` 폴더에 재사용 컴포넌트 생성:
- `Button.tsx` — 기본 / 상승(초록) / 하락(빨강) 변형
- `Badge.tsx` — 섹터 배지, 랭킹 배지
- `Card.tsx` — 종목 카드 공통 래퍼
- `ProgressBar.tsx` — 퀴즈 진행률 바
...
첫 로그인 시 표시:
1. "Newstock이 뭔가요?" 서비스 소개
2. "어떻게 하나요?" 3단계 안내 (뉴스 읽기 → 퀴즈 풀기 → 투자하기)
3. 시작하기 버튼
...
**완료된 작업 (건드리지 않아도 됨)**
- 로그인/회원가입, 미들웨어, 페이지 라우팅, 퀴즈 언락 UI

**남은 작업**

1. `next.config.mjs` 빌드 에러 수정 (김승민A와 협의)
2. `Header.tsx` — 코인 수, 스트릭을 DB에서 실제로 불러오도록 연결 (이은우 DB 완료 후)
3. `quiz/[stock]/page.tsx` — 퀴즈 화면을 뉴스 API와 연결 (김성준 API 완료 후)
4. `portfolio/page.tsx` — 모의 투자 화면 완성 (김성준 trade-logic 완료 후)
...
```
Week 1
  이은우: DB 스키마 전체 완성 → dev 머지
  김승민A: 빌드 에러 수정

Week 2
  김성준: 뉴스 수집 API + 퀴즈 채점 API (병렬 진행)
  허조영: 디자인 시스템 + 온보딩 UI
...
- `main` 직접 push 금지
- PR 전에 반드시 `dev` 최신 내용 pull 받고 충돌 해결 후 올리기
...
`.env.example`을 복사해서 `.env.local` 만들고 각자 키 입력
